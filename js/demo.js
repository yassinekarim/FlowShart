jsPlumb.ready(function() {
    ...
    instance.doWhileSuspended(function() {
        ...
        // instance.connect({uuids:["Window2LeftMiddle",
        // "Window4LeftMiddle"], editable:true});
        // instance.connect({uuids:["Window4TopCenter",
    ...
    function clickFunction(e,box) {
        ...
        inputNom.prop("value", "");
        inputNom.prop("value", box
                .attr("nom"));
        inputDateFin.prop("value",
        "");
        inputDateFin
        .prop("value",box.attr("dateFin"));
        ...
        drop : function(event, ui) {
            if (ui.draggable.hasClass('tache')) {
                var dropped = $('<div style="position:absolute;left:'
                        ...
                        + 'px ;top:'
                        + (ui.position.top)
                        + 'px;" class="node window" id="flowchartwindow'
                        + compteur
                        + '"  test=""></div>');
                ...
                _addEndpoints("window" + compteur, [ "RightMiddle" ], ["TopCenter","LeftMiddle", "BottomCenter" ]);
                compteur++;
                dropped.click();
                ...
                var dropped = $('<div style="position:absolute;left:'
                        ...
                        + 'px ;top:'
                        + (ui.position.top)
                        + 'px;"    id="flowchartstart'
                        + compteur
                        + '" class="node start2"></div>');
                // dropped.draggable();
                $(this).append(dropped);
                instance.draggable($("#flowchartstart"
                        + compteur), {
                    grid : [ 20, 20 ]
                });
                dropped
                ...
                // instance.draggable($("#flowchartstart"+compteur),{
                // grid: [20, 20] });
                /*
                 * $("#flowchartstart"+compteur).draggable( {
                 * drag: function(){ //
                 * instance.repaintEverything();
                 * $(this).setSelected(); var
                 * input=$("#detailEtape");
                 * input[0]=$(this).getAttribute("test"); }
                 * });
                 */
                _addEndpoints("start" + compteur,
                        [ "RightMiddle" ], []);
                compteur++;
                dropped.click();
                ...
                $(this).append(dropped);
                instance.draggable($("#flowchartstop"
                        + compteur), {
                    grid : [ 20, 20 ]
                });
                _addEndpoints("stop" + compteur, [],
                        [ "LeftMiddle" ]);
                compteur++;
                dropped.click();
    ...
    /*
     * function saveFlowchart(){ var nodes = [] $(".node").each(function
     * (idx, elem) { var $elem = $(elem); var endpoints =
     * jsPlumb.getEndpoints($elem.attr('id')); console.log('endpoints of
     * '+$elem.attr('id')); console.log(endpoints); nodes.push({
     * blockId: $elem.attr('id'), nodetype: $elem.attr('data-nodetype'),
     * positionX: parseInt($elem.css("left"), 10), positionY:
     * parseInt($elem.css("top"), 10) }); }); var connections = [];
     * $.each(jsPlumb.getConnections(), function (idx, connection) {
     * connections.push({ connectionId: connection.id, pageSourceId:
     * connection.sourceId, pageTargetId: connection.targetId }); });
     * 
     * var flowChart = {}; flowChart.nodes = nodes;
     * flowChart.connections = connections; flowChart.numberOfElements =
     * numberOfElements;
     * 
     * var flowChartJson = JSON.stringify(flowChart);
     * //console.log(flowChartJson);
     * 
     * $('#jsonOutput').val(flowChartJson); } function loadFlowchart(){
     * var flowChartJson = $('#jsonOutput').val(); var flowChart =
     * JSON.parse(flowChartJson); var nodes = flowChart.nodes;
     * $.each(nodes, function( index, elem ) { if(elem.nodetype ===
     * 'startpoint'){ repositionElement('startpoint', elem.positionX,
     * elem.positionY); }else if(elem.nodetype === 'endpoint'){
     * repositionElement('endpoint', elem.positionX, elem.positionY);
     * }else if(elem.nodetype === 'task'){ var id =
     * addTask(elem.blockId); repositionElement(id, elem.positionX,
     * elem.positionY); }else if(elem.nodetype === 'decision'){ var id =
     * addDecision(elem.blockId); repositionElement(id, elem.positionX,
     * elem.positionY); }else{ } });
     * 
     * var connections = flowChart.connections; $.each(connections,
     * function( index, elem ) { var connection1 = jsPlumb.connect({
     * source: elem.pageSourceId, target: elem.pageTargetId, anchors:
     * ["BottomCenter", [0.75, 0, 0, -1]]
     * 
     * }); });
     * 
     * numberOfElements = flowChart.numberOfElements; } function
     * repositionElement(id, posX, posY){ $('#'+id).css('left', posX);
     * $('#'+id).css('top', posY); jsPlumb.repaint(id); }
     */
});